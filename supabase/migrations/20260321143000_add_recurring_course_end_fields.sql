alter table public.courses
  ...
  add column if not exists end_scheduled_at timestamptz,
  add column if not exists end_reason text;

alter table public.course_registration_intents
  add column if not exists subscription_end_scheduled_at timestamptz,
  add column if not exists course_end_notification_sent_at timestamptz;
