...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'courses_kind_mvp_check'
      and conrelid = 'public.courses'::regclass
  ) then
    alter table public.courses
      add constraint courses_kind_mvp_check
      check (kind in ('course', 'workshop', 'exclusive_offer'));
  end if;
end
$$;
...
select
  ...
  case
    ...
    when c.kind in ('workshop', 'exclusive_offer') then c.starts_at is not null and c.starts_at >= now()
