...
import { type DashboardOfferView, getDisplayStatus } from "./display-status";
...
export default async function DashboardCoursesPage({
  ...
  const { data: profileImage } = await supabase
    .from("profiles")
    .select("company_logo_url,photo_url")
    .eq("id", user.id)
    .maybeSingle<ProfileImageRow>();
  ...
  const fallbackSelect =
    "id,teacher_id,title,description,kind,is_published,visibility,location,location_details,instructor_name,offer_image_url,starts_at,ends_at,duration_minutes,capacity,weekday,start_time,recurrence_type,created_at,cancellation_model,workshop_storno_policy,archived_at,price_cents,currency";
  ...
  for (const row of trialReservationRows) {
    if (row.archived_at || row.cancelled_at || row.decision_status === "rejected") continue;
  ...
  const courseOverviewItems: CourseOverviewItem[] = offers.map((offer) => {
    ...
    return {
      ...
      instructorName: offer.instructor_name,
      ...
      sortKind: isOneTimeOfferKind(kind) ? "einmaliges Angebot" : "laufendes Angebot",
  ...
  return (
    <main className="mx-auto max-w-6xl space-y-6 p-6">
      ...
      {offers.length === 0 ? (
        ...
        <CourseOverviewClient items={courseOverviewItems} initialView={selectedView} />
      )}
    </main>
  );
}
